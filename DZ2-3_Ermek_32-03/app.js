...
function takeOrder(beverageType, itemName) {
    ...
    if (Starbucks[beverageType] && Starbucks[beverageType][itemName]) {
        console.log(`Заказ принят! Вы выбрали  ${itemName}. Описание: ${Starbucks[beverageType][itemName]}`);
    } else {
        console.log("Error: Этот напиток отсутствует в меню.");
    }
}
takeOrder('coffee', 'americano')
takeOrder('tea', 'greenTea')
takeOrder('smoothie', 'berrySmoothie')